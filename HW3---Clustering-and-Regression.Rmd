...
credit_schema <- sapply(read_csv(data_path, n_max = 10), class)

## next call spark_read_csv() using the train_schema
credit <- spark_read_csv(sc,data_path ,columns = credit_schema)
credit
...
credit <- credit %>% 
  ...
  mutate(NumberOfRegularCreditLinesAndLoans = NumberOfOpenCreditLinesAndLoans - NumberRealEstateLoansOrLines) %>% 
mutate(Available_Credit_Percentage = round(((1- RevolvingUtilizationOfUnsecuredLines)*100))) %>%
mutate(
  totalPastDue = 
    NumberOfTime3059DaysPastDueNotWorse * 45 + 
    NumberOfTime6089DaysPastDueNotWorse * 75 + 
...
#mutate(Total_Credit_available = DebtRatio * MonthlyIncome * (1-1/RevolvingUtilizationOfUnsecuredLines))
...
#HERE I USED ALL THE PREDICTORS AVAILABLE IN THE DATASET, THE RMSE VALUE IS 8698.012
```
...
formula <- "MonthlyIncome ~ RevolvingUtilizationOfUnsecuredLines + NumberOfDependents + totalPastDue + DebtRatio + Available_Credit_Percentage + NumberOfTimes90DaysLate + NumberRealEstateLoansOrLines + NumberOfTime6089DaysPastDueNotWorse + age + NumberOfOpenCreditLinesAndLoans "
credit
...
#HERE THE PREDICTORS I USED NUMBER OF DEPENDENTS, TOTAL PAST DUE DEBT RATIO AND ANOTHER MUTATED COLUMN AVAILABLE CREDIT FOR A PERSON IN PERCENTAGE,THE RMSE VALUE IS 8697.906
```
### Your Requirements
...
## change this formula to try different predictors in the model
## TODO: TRY DIFFERENT PREDICTORS HERE
formula <- "MonthlyIncome ~ age + totalPastDue + NumberOfOpenCreditLinesAndLoans + Available_Credit_Percentage "
...
ml_fit(credit_pipeline, partitioned_credit$training) %>%
  ...
  ml_regression_evaluator() 
credit
#HERE THE PREDICTORS I USED AGE, TOTAL PAST DUE, NUMBER OF OPEN CREDIT LINES AND LOANS AND AVAILABLE CREDIT BALANCE IN PERCENTAGE,THE RMSE VALUE IS 8838.158
...
#filter(!is.na(Available_Credit_Percentage), !is.na(NumberOfRegularCreditLinesAndLoans), !is.na(totalPastDue))
  ## add feature generation
...
## change this formula to try different predictors in the model
## TODO: TRY DIFFERENT PREDICTORS HERE
formula <- "SeriousDlqin2yrs ~ totalPastDue + NumberOfRegularCreditLinesAndLoans + NumberOfOpenCreditLinesAndLoans"
credit
## create pipeline (empty)
credit_pipeline <- ml_pipeline(sc) %>%
  ft_dplyr_transformer(tbl = df) %>%
...
## change this formula to try different predictors in the model
## TODO: TRY DIFFERENT PREDICTORS HERE
formula <- "SeriousDlqin2yrs ~ totalPastDue:NumberOfRegularCreditLinesAndLoans + NumberOfOpenCreditLinesAndLoans + age + NumberOfDependents + Available_Credit_Percentage + DebtRatio"
credit
## create pipeline (empty)
credit_pipeline <- ml_pipeline(sc) %>%
  ft_dplyr_transformer(tbl = df) %>%
...
0.7645042
The predictors I used in this model totalPastDue:NumberOfRegularCreditLinesAndLoans, NumberOfOpenCreditLinesAndLoans, age , NumberOfDependents , Available_Credit_Percentage , DebtRatio so highest area under ROC curve value is 0.7645042
# Challenge
